...
static  void dpPrint(const insertionGraph &theGraph,
...
int  doDijkstra(pgr_edge_t  * edges, long total_tuples,
   ...
   try {
        REG_SIGINT
        insertionGraph theGraph = insertionGraph();
	...
	THROW_ON_SIGINT
        DijkstraShortestPathAlg algorithmGraph(theGraph);
	THROW_ON_SIGINT

        log << "NOTICE: Step 3: Getting the paths \n";
        BasePath paths;
        paths = algorithmGraph.Dijkstra(start_vertex, end_vertex, true);
	THROW_ON_SIGINT
...
return 0;
        ...
        pgr_path_element3_t *result_path;
        result_path = 0;
        result_path = pgr_get_memory3(count, result_path);

        int sequence = 0;
        dpPrint(theGraph, paths, result_path, sequence, 0);
